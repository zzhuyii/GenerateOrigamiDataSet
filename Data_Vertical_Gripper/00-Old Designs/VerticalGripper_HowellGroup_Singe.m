...
plotFlag=1; % determine if ploting is needed
...
L1=0.006169; % arm length
L2=0.003919; % arm width
t1=0.216*10^-6; % thickness of two layer
t2=0.614*10^-6; % thickness of two layer
tpanel=27*10^-6; % thickness of panel
rate=0;
W=0.000287; % width of crease
...
ori.panelThickVec=tpanel*ones(9,1); 
ori.panelW=W;
...
UmodeBase=reshape(UmodeBase,3,77)'/10000;
...
for i=1:MaximumLoadingStep
    ...
    node1=squeeze(ori.newNode(16,:))'+squeeze(UhisAssemble(i,16,:));
    node2=squeeze(ori.newNode(20,:))'+squeeze(UhisAssemble(i,20,:));
    distance=norm(node1-node2);
    % Solve the average temperature of crease
    Tmax=max(tempHisAssemble(:,i));
    finalQ=(8/3/sqrt(3)*L2+4*L1+2*range+2/3*L2)*qload*(i-17);      
    finalStep=i;
