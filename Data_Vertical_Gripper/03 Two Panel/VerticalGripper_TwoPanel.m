...
output=readmatrix('Two Panel.txt');
A=size(output);
existingSample=A(1);
...
Ubefore = nr1.Uhis(1,11,2);
Uafter=nr1.Uhis(nr1.increStep,11,2);
...
if finalStep~=maxLoadingStep
    output(tempSampleNum,:)=outputVec;
    tempSampleNum=tempSampleNum+1;
end
writematrix(output,'Two Panel.txt')
end
