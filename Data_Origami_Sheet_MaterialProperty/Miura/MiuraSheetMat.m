...
output=readmatrix('MiuraSheetMat.txt');
A=size(output);
existingSample=A(1);
...
panelE=(rand*4+1)*10^9; % Young's Modulus of panel
creaseE=(rand*4+1)*10^9; % Young's Modulus of crease
...
outputVec=[1 m n tcrease tpanel W creaseE panelE bendstiff axialstiff];
output(tempSampleNum,:)=outputVec;
tempSampleNum=tempSampleNum+1;

writematrix(output,'MiuraSheetMat.txt')
